...
export default function TownshipProjectDetailPage({ params }: PageProps) {
  const { slug } = use(params);

  // Retrieve the specific project data
  const project = townshipProjects[slug];
  ...
  return (
    <main className="w-full min-h-screen bg-[#FDFCFB]">
      ...
      >
        {/* 1. Hero Banner (Links back to Township index page) */}
        <ProjectHero
          name={project.name}
          ...
          backUrl="/services/township"
          backLabel="Back to Townships"
        />
